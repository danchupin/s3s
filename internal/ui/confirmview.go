...
// Confirmation surfaces. The surface is chosen per tier, not per action:
...
//
// Both surfaces carry the loud write badge and share one visual style, and
// cancel on Esc with no mutation. The byte-exact match + y/N logic lives in
...
// caution palette, padded, width-capped — reuses existing tokens.
...
// carries the loud write badge and shares the palette with the inline form.
// It is sized to fit (no clip) and centered in the (w × h) body box.
...
// so a long path / bucket name stays legible. Returns a multi-line block.
